...
;( function( $, window, undefined ) {
	...
	$.Baraja.prototype = {
		...
		_init : function( options ) {
			...
			this.normalParentH = this.$parent.height();
			this.normalParentW = this.$parent.width();*/
		...
		/*_parentSetParamsNormal: function(){
				this.$parent.animate({width: this.normalParentW + 'px', height: this.normalParentH + 'px'}, this.speed)
				this.$parent.parent().animate({scrollLeft: 0, scrollTop: 0}, this.speed, function(){
					this.$parent.parent().css('overflow-x', 'auto')
				})
		},*/
		_validateDefaultFanSettings : function( settings ) {
			var maxTranslationHorizontal = $(window).width()*0.7;
			var maxTranslationVertical = $(window).height()*0.55;
			/*var k = 4, $i = $(this.$items[0]);
			var maxTranslationHorizontal = $i.width()/k * this.itemsCount;
			var maxTranslationVertical = $i.height()/k * this.itemsCount;*/
			var maxTranslation;
			...
			if(!settings.autoDetect){
				...
				var maxTranslation = maxTranslationHorizontal;
			}
			else{
				if(maxTranslationHorizontal>=maxTranslationVertical){
					settings.horizontal = false;
					var maxTranslation = maxTranslationHorizontal;
					settings.origin = {x: this.fanSettings.origin.x, y: this.fanSettings.origin.y};
					settings.selftranslation = maxTranslation*-0.5;
				/*	this._parentSetParams = function(){
						if(maxTranslation<(($(window).height()-$('header').height())*0.7)) return
						$parent.parent().css('overflow-x', 'scroll')
						$parent.animate({width: maxTranslation*1.2 + 'px'}, this.speed);
						$parent.parent().animate({scrollLeft: maxTranslation*1.2 + 'px'}, this.speed)
					}*/
				}
				else{
					settings.horizontal = true;
					var maxTranslation = maxTranslationVertical;
					settings.origin = {y: this.fanSettings.origin.x, x: this.fanSettings.origin.y};
					settings.selftranslation = '0, ' + maxTranslation*-0.5;
					/*this._parentSetParams = function(){
						if(maxTranslation<$(window).width()*0.8) return
						$parent.parent().css('overflow-x', 'scroll')
						$parent.animate({height: maxTranslation*1.2 + 'px'}, this.speed)}
						$parent.parent().animate({scrollTop: maxTranslation*1.2 + 'px'}, this.speed)*/
				}
			}			
			...
			settings.translation = maxTranslation;
			settings.selfmove = true;
		...
		_move2front : function( $item ) {
			...
			/*function _getNextItem(z){
				var nextZ = z -1;
				console.log(nextZ + ';' + z)
				var nextFilter = self.$items.filter(function(n, i){return Number($(i).css('z-index')) == z - 1});
				console.log(nextFilter)
				return nextFilter.length ? nextFilter[0] : null
			}*/

			var self = this,
				z = Number( $item.css( 'z-index' ) ),
				isTop =  z === this.itemZIndexMin + this.itemsCount - 1,
				callback = isTop ? function() { self.isAnimating = false; } : function() { return false; },
				$item = isTop ? null : $item;
				/*$nextItem = $item ? _getNextItem(z) : null;*/
			...
			/*var itemZIndexMax = this.itemsCount + this.itemZIndexMin - 1 ;
			var zIndexCount = itemZIndexMax - z;*/
			/*for (var i = zIndexCount-1; i >= 0; i--) {
				console.log(i)
				self._navigate('next')
			};*/
			/*console.log('REMOVE')
...
/*				if($(this).css('z-index') <=  z){
			...
			var bottom = self.$items.filter(function(n, i){
				if(Number($(i).css('z-index')) < z) return true
				return false
			})

			for (var i = bottom.length - 1; i >= 0; i--) {
				var $i = $(bottom[i]);
				self._updateStack( $i, 'prev' );
			};
		...
		_close : function( callback, $item ) {
			...
			var $el = this.$el;
			this.$el.addClass('first-child-shadow');
			/*this._parentSetParamsNormal()*/
			this._applyTransition( $el, { transform : 'none' }, function(){
		...
		_fan : function( settings ) {
			...
			if(settings.selfmove){
				this._setTransition( this.$el, 'transform', settings.speed, settings.easing );				
				self._applyTransition( this.$el, { transform : 'translate(' + settings.selftranslation + 'px)' });				
			}

			this.$el.removeClass('first-child-shadow');
			/*this._parentSetParams();*/
			...
			this.$items.each( function( i ) {
				...
				if(settings.horizontal){var position = '0, ' + position}
		...
		_add : function( $elems ) {
			...
			// animate new items
			$elems.css( 'transform', 'scale(1.8) translate(150px) rotate(15deg)' ).reverse().each( function( i ) {

				var $el = $( this );

				self._setTransition( $el, 'all', 500, 'ease-out', i * 100 );
