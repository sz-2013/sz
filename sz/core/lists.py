...
def any(condition, list):
    return functools.reduce(
        lambda acc, item: acc or item, map(condition, list), 0)


def all(condition, list):
    return functools.reduce(
        lambda acc, item: acc and item, map(condition, list), 1)
...
def last_match(condition, list):
    return functools.reduce(
        lambda first, second: condition(second) and second or first,
        list, None)
