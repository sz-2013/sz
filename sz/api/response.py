...
class FeedServiceResponseBuilder:
    ...
    def _convert_feed_service_result_to_response(
            self, result, items, viewname, args=None,
            kwargs=None, format=None, **extra):
        ...
        if params.get('category', None) is not None:
            ...
            params['category'] = ""
        url = reverse(
            viewname, args, kwargs, request=self.request,
            format=format, **extra)
...
class PlaceMessagesResponseBuilder(FeedServiceResponseBuilder):
    ...
    def build(self, place, messages):
        photos = [
            (message.id, message.get_photo_absolute_urls(self.photo_host_url))
        ...
        serialized_messages = self._convert_feed_service_result_to_response(
            ...
            'place-detail-messages', (place.pk,))
...
class NewsFeedItemResponseBuilder(FeedServiceResponseBuilder):
    def __init__(self, photo_host_url="", request=None):
        self.messages_response_builder = PlaceMessagesResponseBuilder(
            photo_host_url, request)
        FeedServiceResponseBuilder.__init__(self, request)
...
class NewsFeedResponseBuilder(FeedServiceResponseBuilder):
    def __init__(self, photo_host_url="", request=None):
        self.item_response_builder = NewsFeedItemResponseBuilder(
            photo_host_url, request)
        FeedServiceResponseBuilder.__init__(self, request)
    ...
    def build(self, feed):
        serialized_feed = self._convert_feed_service_result_to_response(
            feed,
            [self.item_response_builder.build(item) for item in feed['items']],
            'place-news')
