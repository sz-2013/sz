...
1)An user do POST a message-data on the url "message-preview-list"
    (MessagePreviewRoot)
2)A client do redirect on "message-preview-publish"
    (MessagePreviewInstancePublish):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if not LEBOWSKI_MODE_TEST:
                ...
                message_preview.user = models.User.objects.get(
                    email=request.DATA.get('email'))
            ...
            serialized_preview['photo'] = \
                message_preview.get_photo_absolute_urls(root_url)
